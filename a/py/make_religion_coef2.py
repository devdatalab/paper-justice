...
for p in ax.patches:
    ...
    if p.get_width() > 0:
        ax.annotate(note, (p.get_width() + 0.004, p.get_y()+.15), fontsize=26, fontweight="normal", color="black")
    else:
        ax.annotate(note, (p.get_width() - 0.017, p.get_y()+.15), fontsize=26, fontweight="normal", color="black")
...
# save figure
plt.savefig(os.path.join(OUT, "r_coef2.png"), bbox_inches="tight", dpi=150)
