...
for p in ax.patches:
    ...
    if p.get_width() > 1:
        ax.annotate(note, (p.get_width() + 0.05, p.get_y()+.2), fontsize=26, fontweight="bold", color="black")
    else:
        ax.annotate(note, (p.get_width() - 0.12, p.get_y()+.15), fontsize=26, fontweight="bold", color="black")

# format axes
ax.tick_params(axis="x", labelsize=26)
ax.set_xlim([0,1.2])
ax.set_ylim([-0.8,12])
ax.set_title("A: Female accused \% : Female population \%", fontsize=36, fontweight = "bold", color="black")
