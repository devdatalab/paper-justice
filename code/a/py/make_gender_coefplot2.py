...
for p in ax.patches:
    ...
    if p.get_width() > 0:
        ax.annotate(note, (p.get_width() + 0.0009, p.get_y()+.15), fontsize=26, fontweight="bold", color="black")
    else:
        ax.annotate(note, (p.get_width() - 0.007, p.get_y()+.15), fontsize=26, fontweight="bold", color="black")

# format axes
ax.tick_params(axis="x", labelsize=26)
ax.set_xlim([-0.025,0.025])
ax.set_ylim([-0.8,12])
ax.set_title("B: Female $-$ male conviction \%", fontsize=36, fontweight = "bold", color="black")
